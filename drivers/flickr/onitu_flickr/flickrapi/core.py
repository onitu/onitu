...
# REST parsers, {format: (parser_method, request format), ...}.
# Fill by using the @rest_parser(format) function decorator
rest_parsers = {}
...
def authenticator(method):
    ...
    Only calls the wrapped method if the token cache doesn't contain a
    valid token.
    '''
...
class FlickrAPI(object):
    ...
    def __init__(self, api_key, secret, username=None,
        ...
        format
            ...
            parsed response, or use any response format supported by Flickr to
            get an unparsed response from method calls. It's also possible to
            pass the ``format`` parameter on individual calls.
    ...
    def parse_xmlnode(self, rest_xml):
        ...
        err = rsp.err[0]
        code = err['code']
        raise FlickrError(six.u('Error: %(code)s: %(msg)s') % (code, err))
    ...
    def parse_etree(self, rest_xml):
        '''Parses a REST XML response from Flickr into an ElementTree
        object.'''
    ...
    def __getattr__(self, method_name):
        ...
        # Compatibility with old way of calling,
        # i.e. flickrobj.photos_getInfo(...)
        if '_' in method_name:
    ...
    def _supply_defaults(self, args, defaults):
        '''Returns a new dictionary containing ``args``, augmented with
    ...
    def upload(self, filename, fileobj=None, **kwargs):
        ...
        return self._upload_to_form(self.UPLOAD_URL, filename, fileobj,
                                    **kwargs)
    ...
    def replace(self, filename, photo_id, fileobj=None, **kwargs):
        ...
        return self._upload_to_form(self.REPLACE_URL, filename, fileobj,
                                    **kwargs)
    ...
    def _upload_to_form(self, form_url, filename, fileobj=None, **kwargs):
        ...
        return self._wrap_in_parser(self.flickr_oauth.do_upload,
                                    response_format, filename, form_url,
                                    kwargs, fileobj)

    def token_valid(self, perms='read'):
        '''Verifies the cached token with Flickr.

        If the token turns out to be invalid, or with permissions lower than
        required,
        the token is erased from the token cache.

        @return: True if the token is valid for the requested parameters,
        False otherwise.
        '''
    ...
    def authenticate_console(self, perms='read'):
        '''Performs the authentication/authorization, assuming a console
        program.

        Shows the URL the user should visit on stdout, then waits for the user
        to authorize
        the program.
        '''
        ...
        print("Go to the following link in your browser to authorize this"
              " application:")
        print(authorize_url)
        print()

        verifier = self.auth_http_server.wait_for_oauth_verifier()
    ...
    def authenticate_via_browser(self, perms='read'):
        ...
        Starts the browser and waits for the user to authorize the app before
        continuing.
        '''
    ...
    def get_request_token(self, oauth_callback=None):
        ...
        Updates this OAuthFlickrInterface object to use the request token on
        the following
        authentication calls.

        @param oauth_callback: the URL the user is sent to after granting the
        token access.
            If the callback is None, a local web server is started on a random
        port, and the
            callback will be http://localhost:randomport/

            If you do not have a web-app and you also do not want to start a
        local web server,
            pass oauth_callback='oob' and have your application accept the
        verifier from the
            user instead.
        '''
    ...
    def auth_url(self, perms='read'):
        ...
        In stand-alone apps, authenticate_via_browser(...) may be easier
        instead.
        '''
    ...
    def get_access_token(self, verifier=None):
        ...
        Also stores the access token for easy authentication of subsequent
        calls.

        @param verifier: the verifier code, in case you used out-of-band
        communication
    ...
    def walk_set(self, photoset_id, per_page=50, **kwargs):
        ...
        return self.data_walker(self.photosets_getPhotos,
                                photoset_id=photoset_id, per_page=per_page,
                                **kwargs)
    ...
    def walk_user_updates(self, min_date, per_page=50, **kwargs):
        ...
        return self.data_walker(self.photos_recentlyUpdated,
                                min_date=min_date, per_page=per_page,
                                **kwargs)
