...
class TokenCache(object):
    ...
    token = property(get_cached_token, set_cached_token, forget,
                     "The cached token")
...
class OAuthTokenCache(object):
    ...
    def create_table(self):
        ...
        # Check DB version
        curs.execute('CREATE TABLE IF NOT EXISTS oauth_cache_db_version '
        ...
        if not oauth_cache_db_version:
            curs.execute('INSERT INTO oauth_cache_db_version (version)'
                         'values (?)', str(self.DB_VERSION))
    ...
    def token(self, token):
        ...
        curs.execute('''INSERT OR REPLACE INTO oauth_tokens
                     (api_key, lookup_key, oauth_token, oauth_token_secret,
                     access_level, fullname, username, user_nsid)
                     values (?, ?, ?, ?, ?, ?, ?, ?)''',
                     (self.api_key, self.lookup_key,
    ...
    def token(self):
        ...
        db = sqlite3.connect(self.filename)
        curs = db.cursor()
        curs.execute('''DELETE FROM oauth_tokens WHERE api_key=?
        and lookup_key=?''',
                     (self.api_key, self.lookup_key))
        db.commit()
